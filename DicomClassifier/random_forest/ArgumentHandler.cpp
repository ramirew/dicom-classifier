...
// Constructor de ArgumentHandler, inicializa las variables miembro con valores por defecto o argumentos pasados
ArgumentHandler::ArgumentHandler(int argc, char **argv) :
    argc(argc), argv(argv), caseweights(""), depvarname(""), fraction(0), holdout(false), memmode(MEM_DOUBLE), savemem(false), skipoob(false), predict(
...
// Función para procesar los argumentos pasados al programa
int ArgumentHandler::processArguments() {

    // Opciones cortas para getopt
    const char *short_options = "A:C:D:F:HM:NOP:Q:R:S:U:XZa:b:c:d:f:hi:j:kl:m:o:pr:s:t:uvwy:z:";

    // Opciones largas para getopt
    ...
    while (1) {
        ...
        // Detener si no quedan más opciones
        if (c == -1) {
            break;
        }
        ...
        case 'M':
            try {
                memmode = static_cast<MemoryMode>(std::stoi(optarg));
        ...
        case 'i':
            try {
                impmeasure = static_cast<ImportanceMode>(std::stoi(optarg));
    ...
    // Imprimir todos los demás parámetros
    while (optind < argc) {
...
// Función para verificar los argumentos y asegurar que sean correctos
void ArgumentHandler::checkArguments() {
    ...
    // Obtener el tipo de árbol para la predicción
    if (!predict.empty()) {
        std::ifstream infile;
        ...
        // No leer los nombres de las variables dependientes
        ...
        // No leer el número de árboles
        infile.ignore(sizeof(size_t));

        // No leer la variable is_ordered
        size_t length;
        infile.read(reinterpret_cast<char*>(&length), sizeof(length));
        infile.ignore(length * sizeof(bool));

        // No leer el número de variables
        infile.ignore(sizeof(size_t));

        // Obtener el tipo de árbol
    ...
    // Verificar modo de holdout
    if (holdout && caseweights.empty()) {
        throw std::runtime_error("Case weights required to use holdout mode.");
    }

    // División no ordenada en survival solo disponible para logrank o extratrees splitrule
    ...
    // Opción de ahorro de memoria no permitida en modo extratrees no ordenado
    ...
    // Importancia de impureza corregida no permitida si se usan pesos de división
