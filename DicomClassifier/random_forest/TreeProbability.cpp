...
// Constructor para inicializar TreeProbability con valores de clase, IDs de respuesta, IDs de muestras por clase y pesos de clase
...
// Reservar memoria para los contadores si no se está en modo de ahorro de memoria
...
// Añadir nodos terminales al árbol
...
// Función para añadir nodos terminales al archivo
...
bool TreeProbability::splitNodeInternal(size_t nodeID, std::vector<size_t>& possible_split_varIDs) {
    ...
    // Comprobar si el nodo es puro y detenerse si lo es
    bool pure = true;
    double pure_value = 0;
    for (size_t pos = start_pos[nodeID]; pos < end_pos[nodeID]; ++pos) {
        size_t sampleID = sampleIDs[pos];
    ...
    // Encontrar la mejor división, detenerse si no hay disminución de la impureza
    bool stop;
...
// Calcular la precisión de la predicción internamente
...
// Encontrar la mejor división para un nodo dado
...
// Encontrar el mejor valor de división para variables ordenadas con pocos valores únicos
...
// Helper function para findBestSplitValueSmallQ
void TreeProbability::findBestSplitValueSmallQ(size_t nodeID, size_t varID, size_t num_classes,
                                               const std::vector<size_t>& class_counts, size_t num_samples_node, double& best_value, size_t& best_varID,
...
// Encontrar el mejor valor de división para variables ordenadas con muchos valores únicos
...
// Helper function para findBestSplitValueExtraTrees
void TreeProbability::findBestSplitValueExtraTrees(size_t nodeID, size_t varID, size_t num_classes,
                                                   const std::vector<size_t>& class_counts, size_t num_samples_node, double& best_value, size_t& best_varID,
...
void TreeProbability::findBestSplitValueExtraTreesUnordered(size_t nodeID, size_t varID, size_t num_classes,
    ...
    for (size_t i = 0; i < num_random_splits; ++i) {
        ...
        if (indices_in_node.size() > 1) {
            size_t num_partitions = (2ULL << (indices_in_node.size() - 1ULL)) - 2ULL;
        ...
        if (indices_out_node.size() > 1) {
            size_t num_partitions = (2ULL << (indices_out_node.size() - 1ULL)) - 1ULL;
...
// Añadir la importancia de impureza para el nodo y variable dados
...
// Muestreo bootstrap por clase sin reemplazo
