...
#include <string>
#include <cmath>
#include <memory> // Incluye <memory> para std::unique_ptr y std::make_unique
...
#include "random_forest/Data.h"
    ...
    // Carga el bosque desde archivos con los datos del modelo previamente entrenado
    void ForestRegression::loadForest(size_t num_trees,
                                      std::vector<std::vector<std::vector<size_t>> >& forest_child_nodeIDs,
                                      std::vector<std::vector<size_t>>& forest_split_varIDs, std::vector<std::vector<double>>& forest_split_values,
                                      std::vector<bool>& is_ordered_variable) {

        this->num_trees = num_trees; // Número de árboles en el bosque
        data->setIsOrderedVariable(is_ordered_variable); // Configura las variables como ordenadas o no

        // Crea los árboles
        trees.reserve(num_trees);
        for (size_t i = 0; i < num_trees; ++i) {
            trees.push_back(std::unique_ptr<TreeRegression>(new TreeRegression(forest_child_nodeIDs[i], forest_split_varIDs[i], forest_split_values[i])));
        }

        // Crea los rangos de hilos
        equalSplit(thread_ranges, 0, num_trees - 1, num_threads);
    }

    // Inicializa parámetros internos del bosque de regresión
    void ForestRegression::initInternal() {

        // Si mtry no está establecido, usa la raíz cuadrada del número de variables independientes
        if (mtry == 0) {
            unsigned long temp = sqrt((double) num_independent_variables);
            mtry = std::max((unsigned long) 1, temp);
        }

        // Establece el tamaño mínimo del nodo
        if (min_node_size == 0) {
            min_node_size = DEFAULT_MIN_NODE_SIZE_REGRESSION;
        }

        // Error si se utiliza splitrule beta con datos fuera del rango [0,1]
        ...
        // Ordena los datos si se está en modo de ahorro de memoria
        if (!memory_saving_splitting) {
            data->sort();
        }
    }

    // Crece el bosque de regresión
    void ForestRegression::growInternal() {
        trees.reserve(num_trees); // Reserva espacio para los árboles
        for (size_t i = 0; i < num_trees; ++i) {
            trees.push_back(std::unique_ptr<TreeRegression>(new TreeRegression())); // Crea cada árbol
        }
    }

    // Asigna memoria para las predicciones
    void ForestRegression::allocatePredictMemory() {
        size_t num_prediction_samples = data->getNumRows(); // Número de muestras para predecir
    ...
    // Realiza la predicción interna para una muestra específica
    void ForestRegression::predictInternal(size_t sample_idx) {
        if (predict_all || prediction_type == TERMINALNODES) {
            // Obtiene todas las predicciones de los árboles
    ...
    // Calcula el error de predicción interna
    void ForestRegression::computePredictionErrorInternal() {

        // Para cada muestra, suma sobre los árboles donde la muestra es OOB
        ...
        // MSE con predicciones y datos reales
    ...
    // Escribe la salida interna del bosque de regresión
    ...
    // Escribe el archivo de confusión
    void ForestRegression::writeConfusionFile() {

        // Abre el archivo de confusión para escribir
        ...
        // Escribe la confusión en el archivo
    ...
    // Escribe el archivo de predicciones
    void ForestRegression::writePredictionFile() {

        // Abre el archivo de predicción para escribir
    ...
    void ForestRegression::loadFromFileInternal(std::ifstream& infile) {
        ...
        // Lee el número de variables
        size_t num_variables_saved;
        infile.read((char*) &num_variables_saved, sizeof(num_variables_saved));

        // Lee el tipo de árbol
        TreeType treetype;
        ...
        for (size_t i = 0; i < num_trees; ++i) {
            ...
            // Si la variable dependiente no está en los datos de prueba, lanza un error
            ...
            // Crea el árbol
            trees.push_back(std::unique_ptr<TreeRegression>(new TreeRegression(child_nodeIDs, split_varIDs, split_values)));

        }
    }

    // Obtiene la predicción de un árbol específico para una muestra específica
    ...
    // Obtiene el ID del nodo terminal de la predicción de un árbol específico para una muestra específica
